...
function wireEditor(mdPath, title){
  const els = {
    ...
    token: document.getElementById("token"), setTokenBtn: document.getElementById("setToken"),
    editBtn: document.getElementById("edit"), saveBtn: document.getElementById("save"),
    view: document.getElementById("content"), raw: document.getElementById("editor"),
    editorWrap: document.getElementById("editorWrap"), currentSha: null
  };
  if(els.title) els.title.textContent = title;
  els.token.value = getToken();
  els.setTokenBtn.addEventListener("click", ()=>{ setToken(els.token.value.trim()); els.status.textContent = "Token saved locally."; });
  els.editBtn.addEventListener("click", ()=>{
    const editing = els.editorWrap.style.display === "block";
    if(editing){
      els.view.innerHTML = marked.parse(els.raw.value || "_(blank)_");
      els.editorWrap.style.display = "none"; els.editBtn.textContent="Edit";
    } else {
      els.editorWrap.style.display = "block"; els.editBtn.textContent="Done";
    }
  });
